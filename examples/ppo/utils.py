...
from rl_cfg import AIOlympicsPPORunnerCfg
from rsl_rl_cfg_defs import RslRlOnPolicyRunnerCfg
...
def parse_args() -> tuple[AIOlympicsCfg, RslRlOnPolicyRunnerCfg]:
    args_cli = get_cli()
    ...
    rslrl_cfg = AIOlympicsPPORunnerCfg()
    ...
    if args_cli.num_steps_per_env is not None:
        rslrl_cfg.num_steps_per_env = args_cli.num_steps_per_env

    return aio_cfg, rslrl_cfg


def get_cli() -> argparse.Namespace:
    parser = argparse.ArgumentParser()

    parser.add_argument("--num_envs", type=int, default=1_000)
...
def load_plant_params(num_envs: int, device: torch.device) -> PlantParams:
    design = "design_C.0"
    model = "model_3.0"
    robot = Robot.ACROBOT
